...
train_transform=transforms.Compose([
    ...
    transforms.ColorJitter(), # 色調をランダムに変化
...
train_dataloader=DataLoader(train_dataset,batch_size=32,shuffle=True,worker_init_fn=seed_worker,generator=generator)
test_dataloader=DataLoader(test_dataset,batch_size=32,shuffle=False,worker_init_fn=seed_worker,generator=generator)
...
class CNN(nn.Module):
    ...
    def forward(self,x):
        x=self.features(x)
        x=x.view(x.size(0),-1) # 1次元のベクトル化　クラス分けをする次行のため
        x=self.classifier(x)
        return x
